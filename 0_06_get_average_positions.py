    ...
    for tournament in unique_tournaments:
        ...
        for index, row in tournament_matches.iterrows():
            ...
            try:
                ...
                df_average_positions = sofascore.scrape_player_average_positions(match_id)

                # Verificar si el DataFrame no está vacío
                if df_average_positions.empty:
                    print(f"Advertencia: No se obtuvieron datos para el partido {match_id} en el año {anio}.")
                    continue
                
                df_average_positions = df_average_positions[['name', 'id', 'team', 'averageX', 'averageY', 'pointsCount']]
                # Guardar los datos en un archivo Excel, con una hoja nombrada según match_id
                output_file_path = os.path.join(tournament_output_dir, f"{round_number}_AveragePositions.xlsx")
                ...
                with pd.ExcelWriter(output_file_path, engine='openpyxl', mode='a' if os.path.exists(output_file_path) else 'w') as writer:
                    df_average_positions.to_excel(writer, sheet_name=str(match_id), index=False)
