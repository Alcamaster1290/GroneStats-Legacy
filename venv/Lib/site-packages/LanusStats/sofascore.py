...
class SofaScore:
    ...
    def get_general_match_stats(self, match_url):
        ...
        url = f'api/v1/event/{match_id}/statistics'
        ...
        df = pd.DataFrame()
        for i in range(len(json_data.json()['statistics'][0]['groups'])):
            df_valores = pd.DataFrame(json_data.json()['statistics'][0]['groups'][i]['statisticsItems'])
            df = pd.concat([df,df_valores])
        df = df[['name', 'home', 'homeValue', 'homeTotal','away', 'awayValue', 'awayTotal', 'statisticsType']]
        return df
