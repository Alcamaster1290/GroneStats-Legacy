...
for anio in range(2022, 2025):
    # Definir la ruta de matches con base en liga y año
    ...
    for tournament in unique_tournaments:
        ...
        for index, row in tournament_matches.iterrows():
            ...
            try:
                # Obtener las estadísticas de los jugadores para el partido
                players_df = sofascore.scrape_player_match_stats(match_id)
                ...
                output_file_path = os.path.join(tournament_output_dir, f"{round_number}_Players.xlsx")
                ...
                with pd.ExcelWriter(output_file_path, engine='openpyxl', mode='a' if os.path.exists(output_file_path) else 'w') as writer:
                    players_df.to_excel(writer, sheet_name=str(match_id), index=False)

                print(f"Datos de jugadores guardados para el partido {match_id} en {output_file_path} para el año {anio}.")

            except Exception as e:
                print(f"Error al obtener jugadores para el partido {match_id} en {tournament} - Ronda {round_number}, año {anio}: {e}")
                continue
